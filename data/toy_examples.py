...
def create_geodesic_pairs_circle(N,time_steps, noise = 0):
    ...
    omega = torch.rand((N,1))*(np.pi/50)
    start_tangent_vecs = torch.stack([-1*start_points[:,1] if N > 1 else -start_points[1], start_points[:,0] if N > 1 else start_points[0]], axis = -1)*omega
