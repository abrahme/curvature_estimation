...
def circle_metric_hemisphere_with_n(sample_sizes: List[int], noise_level: List[float], timesteps:int, keep_preds:bool = False, val:bool = True, loss_type:str = "L2", model_type:str = "neural"):
    ...
    latent_space = Hypersphere(dim = n_dims - 1, equip=True)
    for noise in noise_level:
        for num_samps in sample_sizes:
            torch.set_default_dtype(torch.float32)
            trajectories, start_points, start_velo, val_trajectories, val_start_points, val_start_velo, val_trajectories_clean = create_geodesic_pairs_circle_hemisphere(num_samps, timesteps, noise = 1/noise)
            ...
            model, preds = train(input_trajectories = trajectories, initial_conditions=initial_conditions, epochs = 100,  n = n_dims,
            ...
            with torch.no_grad():
                ...
                plot_convergence([torch.reshape(val_predicted_trajectories, (-1, n_dims))], val_sample_basis,n = num_samps, penalty = 0, skip_every = 30, val = val, noise=noise, hemisphere=True, prior = False)
            ...
            if keep_preds:
                plot_convergence(preds, sample_basis, skip_every=30, n = num_samps, penalty = 0, noise=noise, hemisphere=True, prior = False )
...
def circle_metric_with_n(sample_sizes: List[int], noise_level: float,timesteps:int, keep_preds:bool = False, val:bool = True, loss_type:str = "L2", model_type:str = "neural"):
...
def sphere_metric_with_n(sample_sizes: List[int], noise_level: List[float], timesteps:int, keep_preds:bool = False, val:bool = True, loss_type:str = "L2", model_type: str = "neural"):
    ...
    for noise in noise_level:
        for num_samps in sample_sizes:
            torch.set_default_dtype(torch.float32)
            trajectories, start_points, start_velo, val_trajectories, val_start_points, val_start_velo, val_trajectories_clean = create_geodesic_pairs_sphere(num_samps, timesteps, noise = 1/noise)
            sample_basis = torch.reshape(trajectories,(-1, n_dims)) ### only construct basis from whatever points we have 
            val_sample_basis = torch.reshape(val_trajectories_clean,(-1, n_dims))
            initial_conditions = torch.hstack((start_points, start_velo))
            val_initial_conditions = torch.hstack((val_start_points, val_start_velo))
...
def sphere_metric_hemisphere_with_n(sample_sizes: List[int], noise_level: List[float], timesteps:int, keep_preds:bool = False, val:bool = True, loss_type:str = "L2", model_type:str = "neural"):
    ...
    for noise in noise_level:
        for num_samps in sample_sizes:
            torch.set_default_dtype(torch.float32)
            trajectories, start_points, start_velo, val_trajectories, val_start_points, val_start_velo, val_trajectories_clean = create_geodesic_pairs_sphere_hemisphere(num_samps, timesteps, noise = 1/noise)
...
def sphere_autoencoder_with_n(sample_sizes: List[int], noise_level: List[float], timesteps:int, keep_preds:bool = False, val:bool = True, loss_type:str = "L2"):
    losses = []
    n_dims = 3
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_type", type = str, help="which model", default = "neural")
    ...
    if args.manifold == "circle":
        if args.hemisphere:
            if not args.autoencoder:
                circle_metric_hemisphere_with_n(sample_sizes = args.sample_sizes, noise_level = args.noise,  keep_preds=args.keep_preds, timesteps=args.timesteps, loss_type=args.loss, model_type=args.model_type)
            else:
            ...
            if not args.autoencoder:
                circle_metric_with_n(sample_sizes = args.sample_sizes, noise_level = args.noise, keep_preds=args.keep_preds, timesteps=args.timesteps, loss_type=args.loss,model_type=args.model_type)
            else:
        ...
        if args.hemisphere:
            if not args.autoencoder:
                sphere_metric_hemisphere_with_n(sample_sizes = args.sample_sizes, noise_level = args.noise, keep_preds=args.keep_preds, timesteps=args.timesteps, loss_type=args.loss, model_type=args.model_type)
            else:
