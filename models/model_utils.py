...
def compute_frechet_distance(predicted_trajectories, actual_trajectories, model: RiemannianAutoencoder):
    frechet_calculator = DiscreteFrechet(dist_funct = model.metric_space.metric.dist)
    dist = 0
    for pred, actual in zip(predicted_trajectories, actual_trajectories):
        dist += frechet_calculator(pred, actual)
    return dist
